...
class RegistrationForm(forms.Form):
	...
	zipCode = forms.CharField(max_length=5)
	country = forms.ChoiceField(choices=countryList, initial='France', required = True,)
	gender = forms.ChoiceField(choices=genderList, initial='Male', required = True,)
	...
	'''def clean_profilePicture(self):
		imageField = self.cleaned_data.get('profilePicture')
		imageFile = cStringIO.StringIO(imageField.read())
		image = Image.open(imageFile)
		w, h = image.size
		image = image.resize((215,215), Image.ANTIALIAS)
		imageFile = cStringIO.StringIO()
		image.save(imageFile,'JPEG',quality=90)
		imageField.file=imageFile'''
